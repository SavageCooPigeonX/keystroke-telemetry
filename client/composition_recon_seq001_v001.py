"""
composition_recon_seq001_v001.py — Composition Buffer Reconstruction
...
Outputs to logs/composition_recon_seq001_v001.jsonl:
...
Usage: py client/composition_recon_seq001_v001.py <project_root> [--once]
"""
import json
import os
import sys
...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({'error': 'usage: py composition_recon_seq001_v001.py <root> [--once]'}))
        sys.exit(1)

    project_root = sys.argv[1]
    once = '--once' in sys.argv
    log_path = os.path.join(project_root, 'logs', 'composition_recon_seq001_v001.jsonl')
