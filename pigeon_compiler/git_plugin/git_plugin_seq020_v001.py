"""git_plugin_seq020_v001.py — Pigeon-extracted by compiler."""
from .git_plugin_seq001_v001 import _commit_hash
from .git_plugin_seq001_v001 import _commit_msg
...
from .git_plugin_seq002_v001 import _changed_files
from .git_plugin_seq002_v001 import _file_diff_stat
from .git_plugin_seq002_v001 import _parse_intent
from .git_plugin_seq003_v001 import _collect_bug_keys
from .git_plugin_seq003_v001 import _intent_code
from .git_plugin_seq004_v001 import _sync_bug_metadata
from .git_plugin_seq005_v001 import _inject_box
from .git_plugin_seq015_v001 import _run_post_commit_extras
from .git_plugin_seq016_v001 import _load_edit_authors
from .git_plugin_seq016_v001 import _load_edit_whys
from .git_plugin_seq018_v001 import _ROOT_DEBUG
...
def run():
    ...
    if not registry:
        try:
            ...
            print(f'  ⚠️  registry bootstrap: {exc}')
    ...
    for rel in changed:
        ...
        try:
            ...
            map_entry = load_file_intent_map(root, rel)
            enrich_registry_entry(
                entry,
                path=rel,
                intent_key=str(map_entry.get('primary_intent_key') or entry.get('intent_key') or ''),
                last_change=str(entry.get('last_change') or intent),
                event='version_bump',
            )
        except Exception:
            pass

        # Compressed files — prefer legacy semantic name when readable
        if parsed.get('compressed'):
            entry.setdefault('semantic_name', entry.get('semantic_name') or parsed.get('deps') or parsed.get('name'))
            ...
            new_name = None
            try:
                prefer_legacy_filename = src_import("registry_identity_bridge_seq001", "prefer_legacy_filename")

                new_name = prefer_legacy_filename(
                    entry,
                    date=today,
                    desc=desc,
            ...
            if not new_name:
                ...
                entry['name'] = entry.get('semantic_name') or entry.get('name')
        ...
        if entry.get('itid'):
            new_name = build_intent_filename(
                entry.get('identity_id') or parsed['name'],
                entry['itid'],
                entry['ver'],
                entry['date'],
                desc,
    ...
    # Log sessions + inject prompt boxes + alias bridge
    try:
        merge_rename_alias = src_import("registry_identity_bridge_seq001", "merge_rename_alias")
    except Exception:
        merge_rename_alias = None
    for old_rel, new_rel, entry, tb, ds in renames:
        if merge_rename_alias and old_rel != new_rel:
            merge_rename_alias(root, old_rel, new_rel, entry, write=True)
    ...
    # Save registry
    try:
        audit_registry_pairing, patch_registry = src_import("registry_identity_bridge_seq001", "audit_registry_pairing", "patch_registry")

        audit = audit_registry_pairing(root, registry)
        audit_path = root / 'logs/registry_pairing_audit_latest.json'
        audit_path.parent.mkdir(parents=True, exist_ok=True)
        audit_path.write_text(json.dumps(audit, indent=2, sort_keys=True) + '\n', encoding='utf-8')
        if not audit.get('healthy'):
            print(f'  ⚠️  registry pairing: {audit.get("mismatch_count", 0)} mismatch(es)')
        if not registry:
            patch_registry(root, write=True, rebuild=False)
            registry = load_registry(root)
    except Exception as exc:
        print(f'  ⚠️  registry pairing audit: {exc}')
