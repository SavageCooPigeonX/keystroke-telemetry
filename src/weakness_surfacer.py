"""weakness_surfacer — what the codebase is trying to tell you, surfaced.
...
Run: py -m src.weakness_surfacer  (prints + writes logs/weaknesses_now.json)
"""
...
def check_health_score() -> dict | None:
    snap = _read_json(ROOT / "logs" / "push_snapshots" / "_latest.json")
    if snap is None:
...
def check_self_fix_outcomes() -> dict | None:
    log = ROOT / "logs" / "self_fix_verification.jsonl"
    if not log.exists():
        return {"severity": "medium", "msg": "self_fix_verification.jsonl missing — fixes write but never verify"}
