"""consensus_sim.py — multi-file consensus debugging via the organism chat server.
...
Usage:
    py src/consensus_sim.py                     # auto-pick worst chronic bug
    py src/consensus_sim.py --bug overcap       # target specific bug type
    py src/consensus_sim.py --modules a,b,c     # override module selection
    py src/consensus_sim.py --interactive-only   # skip consensus, go straight to chat
...
def _select_modules_for_bug(bug_type: str, bug_data: list[dict],
    ...
    fixer_keywords = {
        ...
        'dead_export': ['self_fix', 'compliance', 'context_veins'],
    }
