"""pitch_sim_seq001_v001.py — optimization sim: modules write their own pitch.
...
Usage:
    py -m src.pitch_sim_seq001_v001_seq001_v001                    # auto-select top entropy module
...
def _load_module_context(name: str) -> dict:
    ...
    bugs = _load_json(ROOT / 'logs' / 'bug_profiles_seq001_v001.json')
...
def _call_gemini(system_prompt: str, user_prompt: str) -> str | None:
    ...
    if not api_key:
        print('[pitch_sim_seq001_v001] ERROR: no GEMINI_API_KEY')
        return None
    ...
    try:
        ...
        print(f'[pitch_sim_seq001_v001] gemini error: {e}')
        return None
...
def run_pitch(module_name: str | None = None, top_n: int = 1):
    ...
    if module_name:
        ...
        if not targets:
            print('[pitch_sim_seq001_v001] no high-entropy modules found in entropy_map.json')
            return

    print(f'[pitch_sim_seq001_v001] targeting {len(targets)} module(s)')
    print()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description='pitch_sim_seq001_v001 — modules write their own pitch')
