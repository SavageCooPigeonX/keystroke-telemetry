"""tc_sim.py — replay typed sessions through the thought completer pipeline.
...
Usage:
    py -m src.tc_sim                    # replay last 5 sessions
    py -m src.tc_sim --n 20             # replay last 20 sessions
    py -m src.tc_sim --live             # replay + call Gemini (costs API)
    py -m src.tc_sim --session 42       # replay specific session index
    py -m src.tc_sim --min-len 20       # only sessions with 20+ char buffer
    py -m src.tc_sim --pause-ms 1200    # custom pause threshold
    py -m src.tc_sim --export sim.jsonl # export results
    py -m src.tc_sim --transcript       # comedy narrative transcript
    py -m src.tc_sim --fix              # identify + apply fixes from sim data
    py -m src.tc_sim --narrate          # plain english explanation of everything
...
def apply_fix(bug: dict, dry: bool = False) -> str:
    ...
    if fix_type == 'deduplicate_registry_results':
        target = ROOT / 'src' / 'tc_context_agent.py'
