"""push_snapshot_seq001_v001_health_score_decomposed_seq012_v001.py — Honest health score.
...
def _veto_caps(snapshot: dict) -> list[tuple[str, int]]:
    ...
    card = _read_json(_ROOT / "logs" / "rework_scorecard_seq001_v001.json")
    if card:
...
def _outcome_component(snapshot: dict) -> float:
    """Actual fix rate from self_fix_verification_seq001_v001.jsonl. Max +15."""
    log = _ROOT / "logs" / "self_fix_verification_seq001_v001.jsonl"
    if not log.exists():
        return 0.0
