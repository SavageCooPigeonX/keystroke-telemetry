...
# ── pigeon ────────────────────────────────────
# SEQ: 001 | VER: v004 | 714 lines | ~6,804 tokens
...
def _registry_canonical_keys(mod: dict) -> set[str]:
    ...
    try:
        from .intent_numeric_seq001_v004_d0420__word_number_file_mapping_for_lc_chore_pigeon_rename_cascade import canonicalize_file_key
    except Exception:
        return set()
...
def select_context_numeric(buffer: str, ctx: dict, max_files: int = 3) -> list[dict]:
    ...
    """
    try:
        from .intent_numeric_seq001_v004_d0420__word_number_file_mapping_for_lc_chore_pigeon_rename_cascade import predict_files, get_stats, canonicalize_file_key
        stats = get_stats()
