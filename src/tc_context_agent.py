...
Section-aware: uses the operator's behavioral section (from tc_profile) to
...
def select_context_numeric(buffer: str, ctx: dict, max_files: int = 3) -> list[dict]:
    ...
    """
    try:
        from .intent_numeric import predict_files, get_stats
        stats = get_stats()
...
def select_context_files(buffer: str, ctx: dict, max_files: int = 3) -> list[dict]:
    ...
    section_style = {}
    try:
        from .tc_profile import classify_section, load_profile
...
def get_intelligent_context(buffer: str, ctx: dict | None = None) -> dict:
    ...
    try:
        from .tc_manifest import match_intent_profile, get_tc_context
        tc_ctx = get_tc_context()
...
def log_prediction(buffer: str, predicted_files: list[str], 
    ...
    try:
        from .tc_manifest import match_intent_profile
        name, profile, score = match_intent_profile(buffer)
...
def grade_prediction(prediction_ts: str, actual_intent: str, 
    ...
    try:
        from .tc_manifest import log_prediction as manifest_log, adjust_weight
        manifest_log(
...
def grade_and_learn(buffer: str, completion: str, outcome: str,
    ...
    try:
        from .tc_manifest import update_intent_profile, match_intent_profile
        ...
        if name and score > 0.3:
            ...
            if outcome in ('accepted', 'rewarded') and files_touched:
                ...
                from .tc_profile import extract_session_triggers
