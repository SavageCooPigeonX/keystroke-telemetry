...
Composes existing infrastructure:
  ...
  - node_tester_seq001_v001: dead import / unused export / dead edge checks
  - file_heat_map: cognitive load data
  - pigeon_registry: module metadata
  - dual_substrate: merged human/agent heat

Outputs context_veins_seq001_v001.json consumed by dynamic_prompt for self-trim injection.
"""
...
def analyze_veins(root: Path) -> dict:
    """Run full vein/clot analysis. Returns the context_veins_seq001_v001 structure."""
...
def get_trim_summary(root: Path) -> str:
    """Load context_veins_seq001_v001.json and return a compact summary for prompt injection."""
    p = root / VEINS_PATH
    if not p.exists():
        return ""
    try:
