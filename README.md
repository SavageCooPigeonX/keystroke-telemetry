...
├── logs/
│   ├── prompt_journal.jsonl         ← every Copilot prompt logged
│   ├── prompt_compositions.jsonl    ← keystroke compositions + deleted words
...
| Post-commit pipeline | ✅ Fully wired (9-step auto-commit) |
| Pigeon compiler | ✅ Operational |
| MASTER_MANIFEST | ✅ Rebuilt |
| Tests | ✅ 4/4 passing |

**Known open issues:**
- `stress_test.py`, `test_all.py`, `vscode-extension/pulse_watcher.py` have hardcoded pigeon imports (CRITICAL — will break on next rename of those files)
- 76 `(background)` queries polluting `query_memory.json` (filter in extension flush)
- `operator_stats_seq008*` (397 lines) and `self_fix_seq013*` (352 lines) are candidates for next pigeon compile run
