...
**Operational update, 2026-05-28:** keystroke telemetry is now functioning as a MAIF maintenance substrate, not only a typing-signal experiment. Local intent-key work routes Hush, IRT/artifact probes, Pigeon compiler state, private profile signals, and keystroke context into domain-scoped file memory. Treat this repo as a full-time maintenance layer for cross-project context routing: preserve local telemetry WIP before pulling, route generated code through intent profiles, and keep raw prompt, typing, deletion, and MAIF data as private local evidence rather than public telemetry output.

**Operator data storage rule, 2026-06-28:** raw prompts, keystrokes, deleted words, query memory, rework logs, operator profiles, local coaching, and task queues belong in MAIF or ignored local spools, never in git. The mandatory pre-push guard is documented in [`docs/operator_data_storage_contract.md`](docs/operator_data_storage_contract.md) and installed with `py scripts\install_operator_data_guard_hook_seq001_v001__install_pre_push_operator_data_guard_lc_data_storage_operator_happens.py`.
