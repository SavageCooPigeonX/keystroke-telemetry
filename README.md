...
**Operator data storage rule, 2026-06-28:** raw prompts, keystrokes, deleted words, query memory, rework logs, operator profiles, local coaching, and task queues belong in MAIF or ignored local spools, never in git. The mandatory pre-push guard is documented in [`docs/operator_data_storage_contract.md`](docs/operator_data_storage_contract.md) and installed through the combined hook installer: `py scripts\install_pigeon_hooks.py`.
