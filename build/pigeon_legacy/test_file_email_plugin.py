...
def test_touch_email_writes_local_outbox():
    ...
    assert record["to"] == "contact@myaifingerprint.com"
    ...
    assert payload["payload"]["to"] == ["contact@myaifingerprint.com"]
...
def test_file_sim_email_includes_file_comment_and_deepseek_result():
    root = _root()
    job_id = "dsc-test-file-sim"
    (root / "logs" / "deepseek_prompt_results.jsonl").write_text(
        json.dumps({
            "job_id": job_id,
            "success": True,
            "model": "deepseek-test",
            "reason": "deepseek prompt context completed",
            "completion": "Drafted a split plan and named the validation gate.",
        }) + "\n",
        encoding="utf-8",
    )
    ...
    result = emit_file_sim_emails(
        ...
        {
            "trigger": "file_sim",
            "intent": {"intent_key": "src:patch:router:patch"},
            "target_state": "interlinked_source_state",
            "proposals": [{
                "path": "src/router.py",
                "decision": "safe_dry_run",
                "interlink_score": 0.9,
                "proposed_fix": "router says validator needs to wake first",
                "file_comment": "I should not be rewritten until validator is loaded.",
                "deepseek_completion_job_id": job_id,
                "context_injection": ["src/router.py", "src/validator.py"],
                "validation_plan": ["py -m py_compile src/router.py"],
                "ten_q": {"passed": True, "score": 10, "max_score": 10, "reason": "passed"},
                "orchestrator_email_guard": {"aligned": True, "decision": "allow_email"},
            }],
        },
    ...
    assert record["deepseek_receipt"]["status"] == "done"
    assert record["file_comment"] == "I should not be rewritten until validator is loaded."
    latest = (root / "logs" / "file_email_latest.md").read_text(encoding="utf-8")
    assert "My file comment:" in latest
    assert "I should not be rewritten until validator is loaded" in latest
    assert "DeepSeek `dsc-test-file-sim` done" in latest
    assert "Drafted a split plan" in latest
...
def test_file_email_config_migrates_learning_digest_trigger():
    ...
    assert "file_opinion" in saved["triggers"]
