...
def run():
    ...
    if '[pigeon-auto]' in msg:
        return
    h = _run_commit_hash()
    intent = _run_parse_intent(msg)
    changed = _run_changed_files()
    if not changed:
        return
    registry, edit_whys, edit_authors = _run_load_data(root)
    renames, box_only, import_map = _run_process_changed_files(root, changed, intent, registry, edit_whys, edit_authors)
    if not renames and not box_only:
        _run_handle_non_pigeon_changes(root, intent, h, changed, registry, msg)
        return
    _run_print_summary(renames, box_only)
    _run_rewrite_imports(root, import_map)
    _run_execute_renames(root, renames)
    _run_log_and_inject(root, renames, box_only, h, msg)
    save_registry(root, registry)
    processed = len(renames) + len(box_only)
    changed_py = _run_get_changed_py_list(renames, box_only)
    cross_context = _run_self_fix(root, registry, changed_py, intent)
    bug_renames, bug_import_map, bug_path_map = _run_bug_marker_update(root, changed_py, registry)
    _run_rewrite_bug_imports(root, bug_import_map)
    _run_execute_bug_renames(root, bug_renames, registry)
    if bug_path_map:
    ...
    all_changed_code = _run_get_all_changed_code(changed_py, changed)
    _run_post_commit_extras(root, intent, h, all_changed_code, registry, msg, renames, box_only, cross_context)
    _run_prompt_manager_refresh(root, registry, processed)
    _run_rebuild_manifests(root)
    _run_self_fix_accuracy(root)
    _run_context_compression(root, changed)
    _run_intent_compression(root)
    _run_codebase_transmutation(root)
    _run_vitals_snapshot(root, h, msg)
    total_tokens = _run_compute_total_tokens(renames, box_only)
    rename_count = len(renames) + len(bug_renames)
    _run_validate_imports(root, import_map, rename_count)
    _run_autonomous_escalation(root)
    _run_learning_loop_catchup(root)
    _run_auto_commit(root, rename_count, intent, msg, total_tokens)
...
def _run_load_data(root):
    from pigeon.registry import load_registry
    registry = load_registry(root)
    edit_whys = _load_edit_whys(root)
    edit_authors = _load_edit_authors(root)
    return registry, edit_whys, edit_authors

def _run_process_changed_files(root, changed, intent, registry, edit_whys, edit_authors):
    from pigeon.parse import parse_pigeon_stem, extract_desc_slug
    from pigeon.filename import build_pigeon_filename, mutate_compressed_stem
    from pigeon.utils import is_excluded, _estimate_tokens, _file_diff_stat, _intent_code
    from pigeon.registry import bump_version
    renames = []
    box_only = []
    import_map = {}
    today = datetime.now(timezone.utc).strftime('%m%d')
    _ROOT_DEBUG = re.compile(r'^[a-z]_\w+_s\d{3}[a-z]?\.py$')
    for rel in changed:
        p = Path(rel)
        ...
        if p.stem != Path(new_name).stem:
            ...
            box_only.append((abs_p, entry, rel, tokens_before, diff_stat))
    return renames, box_only, import_map

def _run_handle_non_pigeon_changes(root, intent, h, changed, registry, msg):
    all_changed_code = [f for f in changed if f.endswith(('.py', '.ts', '.js'))]
    if all_changed_code:
        print(f'\n🐦 Pigeon Git Plugin: 0 rename(s), 0 update(s), {len(all_changed_code)} non-pigeon file(s)')
        _run_post_commit_extras(root, intent, h, all_changed_code, registry, msg)

def _run_print_summary(renames, box_only):
    print(f'\n🐦 Pigeon Git Plugin: {len(renames)} rename(s), {len(box_only)} update(s)')

def _run_rewrite_imports(root, import_map):
    if import_map:
        from pigeon.import_rewrite import rewrite_all_imports
...
def _run_execute_renames(root, renames):
...
def _run_log_and_inject(root, renames, box_only, h, msg):
    from pigeon.session import log_session
    from pigeon.box import _inject_box
...
def _run_get_changed_py_list(renames, box_only):
    changed_py = [nr for _, nr, _, _, _ in renames] + [r for _, _, r, _, _ in box_only]
    return changed_py

def _run_self_fix(root, registry, changed_py, intent):
    from pigeon.utils import _load_glob_module, _collect_bug_keys, _sync_bug_metadata
    from pigeon.registry import save_registry
    cross_context = {}
    try:
        fix_mod = _load_glob_module(root, 'src', '修f_sf_s013*')
        if fix_mod:
            fix_report = fix_mod.run_self_fix(root, registry, changed_py=changed_py, intent=intent)
            ...
            for path, entry in registry.items():
                ...
                _sync_bug_metadata(entry, bug_keys.get(path, []), datetime.now(timezone.utc).strftime('%m%d'))
            save_registry(root, registry)
...
def _run_bug_marker_update(root, changed_py, registry):
    from pigeon.parse import parse_pigeon_stem
    from pigeon.filename import mutate_compressed_stem, bug_marker_from_keys
    from pigeon.utils import _intent_code
    bug_renames = []
    bug_import_map = {}
    bug_path_map = {}
    today = datetime.now(timezone.utc).strftime('%m%d')
    ...
    return bug_renames, bug_import_map, bug_path_map

def _run_rewrite_bug_imports(root, bug_import_map):
    if bug_import_map:
        from pigeon.import_rewrite import rewrite_all_imports
...
def _run_execute_bug_renames(root, bug_renames, registry):
    from pigeon.registry import save_registry
...
def _run_get_all_changed_code(changed_py, changed):
    all_changed_code = changed_py + [f for f in changed if f.endswith(('.py', '.ts', '.js')) and f not in changed_py]
    return all_changed_code

def _run_post_commit_extras(root, intent, h, all_changed_code, registry, msg, renames=None, box_only=None, cross_context=None):
    pass

def _run_prompt_manager_refresh(root, registry, processed):
    from pigeon.utils import _load_glob_module
    try:
...
def _run_rebuild_manifests(root):
    try:
        from pigeon.manifest import build_all_manifests
...
def _run_self_fix_accuracy(root):
    ...
    try:
        ...
        if sft_mod and hasattr(sft_mod, 'compute_accuracy'):
            ...
            if 'error' not in acc:
                print(f"  📊 self-fix accuracy: {acc['total_threads']} threads, fix rate {round(acc['avg_fix_rate'] * 100, 1)}% ({acc['status_breakdown'].get('resolved', 0)} resolved, {acc['status_breakdown'].get('eternal', 0)} eternal)")
    except Exception as e:
    ...
    try:
        ...
        if cc_mod and hasattr(cc_mod, 'compress_changed'):
            ...
            if cc_result['files'] > 0:
                print(f"  🗜️  compression: {cc_result['orig_tokens']}→{cc_result['compressed_tokens']} tokens ({cc_result['ratio']}x, {cc_result['files']} files, {cc_result['elapsed_ms']}ms)")
    except Exception as e:
...
def _run_intent_compression(root):
    ...
    try:
        ...
        if ic_mod and hasattr(ic_mod, 'compress_all'):
            ...
            print(f"  🧠 intent compression: {ic_result.get('total_original_tokens', 0)}→{l3.get('tokens', 0)} tok skeleton ({l3.get('ratio', 0)}x, {ist.get('intent_amplification', 0)}x intent amp)")
    except Exception as e:
...
def _run_codebase_transmutation(root):
    ...
    try:
        ...
        if ct_mod and hasattr(ct_mod, 'transmute_all'):
            ...
            s = tr.get('stats', {})
            n = tr.get('numerical', {})
            print(f"  🔢 transmute: {s.get('files', 0)} files, {s.get('tokens', 0)} tok, noise {s.get('noise_pct', 0)}%, numerical {n.get('ratio', 0)}x ({n.get('unique_symbols', 0)} symbols)")
    except Exception as e:
