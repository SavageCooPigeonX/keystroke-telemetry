...
Launch:  py src/thought_completer.py
         ...
         py src/thought_completer.py --intent-graph "thought completer should split this into five moves"
         py src/thought_completer.py --prompt-brain "typing buffer to assemble"
         py src/thought_completer.py --dead-token-collective
         py src/thought_completer.py --rewrite-sim "compile intent and propose fixes"
         py src/thought_completer.py --fingerprint-repo C:\\path\\repo --fingerprint-label maif_auditor
         py src/thought_completer.py --sim-buffer "audit stale data points"
         py src/thought_completer.py --unsaid-digest "hook architecture file sim"
...
def main():
    ...
    p.add_argument('--unsaid-digest', nargs='?', const='',
                   help='write low-friction unsaid thread context files for Copilot/Codex and exit')
    p.add_argument('--unsaid-limit', type=int, default=5,
                   help='max threads to include in --unsaid-digest outputs')
    p.add_argument('--intent-key', help='generate a manifest-backed intent key for this prompt and exit')
    p.add_argument('--intent-graph', help='generate five-ish manifest/numeric intent moves for this prompt and exit')
    p.add_argument('--prompt-brain', help='assemble full watcher context bundle for this prompt and exit')
    p.add_argument('--dead-token-collective', action='store_true',
                   help='mine git rename/change history into dead-token training pairs')
    p.add_argument('--history-limit', type=int, default=0,
                   help='max commits for --dead-token-collective; 0 means all history')
    p.add_argument('--rewrite-sim', nargs='?', const='',
                   help='proposal-only batch rewrite sim for this intent')
    p.add_argument('--rewrite-limit', type=int, default=8,
                   help='max proposals for --rewrite-sim')
    p.add_argument('--fingerprint-repo', help='plug an external repo into prompt encoding / AI fingerprint')
    p.add_argument('--fingerprint-label', default='', help='stable label for --fingerprint-repo')
    p.add_argument('--fingerprint-limit', type=int, default=120, help='max files to encode from --fingerprint-repo')
    p.add_argument('--fingerprint-privacy', choices=['closed', 'open'], default='closed',
                   help='closed redacts repo text from telemetry logs; open stores readable terms')
    p.add_argument('--fingerprint-no-train', action='store_true', help='index repo without training numeric matrix')
    ...
    if args.intent_graph:
        ...
        compat = None
        try:
            compat = importlib.import_module("codex_compat")
        except Exception:
            compat = None
        numeric_files = []
        context_selection = {}
        if compat is not None:
            try:
                context_selection = compat.select_context(_Path(_root_dir), args.intent_graph)
            except Exception:
                context_selection = {}
            try:
                numeric_files = compat.predict_numeric_files(_Path(_root_dir), args.intent_graph)
            except Exception:
                numeric_files = []
    ...
    if args.unsaid_digest is not None:
        import json as _json
        from pathlib import Path as _Path
        from src.unsaid_thread_digest_seq001_v001 import build_unsaid_thread_digest
        result = build_unsaid_thread_digest(
            _Path(_root_dir),
            args.unsaid_digest,
            limit=args.unsaid_limit,
            write=True,
        )
        print(_json.dumps(result, indent=2, ensure_ascii=False))
        return 0

    if args.dead_token_collective:
        import json as _json
        from pathlib import Path as _Path
        from src.dead_token_collective_seq001_v001 import collect_dead_token_history
        result = collect_dead_token_history(_Path(_root_dir), max_commits=args.history_limit, write=True)
        print(_json.dumps(result, indent=2, ensure_ascii=False))
        return 0

    if args.rewrite_sim is not None:
        import json as _json
        from pathlib import Path as _Path
        from src.batch_rewrite_sim_seq001_v001 import simulate_batch_rewrites
        result = simulate_batch_rewrites(_Path(_root_dir), args.rewrite_sim, limit=args.rewrite_limit, write=True)
        print(_json.dumps(result, indent=2, ensure_ascii=False))
        return 0

    if args.fingerprint_repo:
        import json as _json
        import re as _re
        from pathlib import Path as _Path
        from src.ai_fingerprint_repo_seq001_v001 import plug_repo
        repo_path = _Path(args.fingerprint_repo)
        label = args.fingerprint_label or _re.sub(r"[^a-zA-Z0-9]+", "_", repo_path.name.lower()).strip("_")
        result = plug_repo(_Path(_root_dir), repo_path, label, args.fingerprint_limit,
                           train=not args.fingerprint_no_train,
                           privacy=args.fingerprint_privacy)
        print(_json.dumps(result, indent=2, ensure_ascii=False))
        return 0
