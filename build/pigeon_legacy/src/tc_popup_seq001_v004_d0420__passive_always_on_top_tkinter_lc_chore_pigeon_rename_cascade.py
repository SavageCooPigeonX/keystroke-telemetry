...
# ── pigeon ────────────────────────────────────
# SEQ: 001 | VER: v004 | 568 lines | ~6,996 tokens
...
from .tc_constants_seq001_v001 import (ROOT, KEYSTROKE_LOG, LOG_PATH, GEMINI_MODEL,
                           POLL_INTERVAL_MS)
...
_TERMINAL_BUFFER_EVENTS = {'submit', 'discard'}


def _completion_candidate_buffer(current_buffer: str, last_nonempty: str, last_event_type: str) -> str:
    """Return the buffer eligible for pause completion.

    A submitted/discarded prompt is terminal. Do not resurrect the last
    non-empty buffer after the watcher has cleared it, or the popup will
    re-run the same prompt whenever the duplicate lock expires.
    """
    current = (current_buffer or '').strip()
    if current:
        return current
    if last_event_type in _TERMINAL_BUFFER_EVENTS:
        return ''
    return (last_nonempty or '').strip()
...
def run_popup(corner='br', pause_ms=1500, width=520, height=220, opacity=0.92, surface_ready=None):
    ...
    class App:
        def __init__(self):
            ...
            self._last_nonempty_buf = self.watcher.buffer.strip()
        ...
        def _poll_tick(self):
            ...
            if changed:
                terminal_event = self.watcher.last_event_type in _TERMINAL_BUFFER_EVENTS
                if buf:
                    self._last_nonempty_buf = buf
                elif terminal_event:
                    self._last_nonempty_buf = ''
                    self._completed_buffer = None
                    self._completed_buffer_ts = 0.0
                    self._del_buf_history.clear()
                if not buf:
                    self._completed_buffer = None
                ...
                check_buf = _completion_candidate_buffer(
                    buf,
                    self._last_nonempty_buf,
                    self.watcher.last_event_type,
                )
        ...
        def _refresh_display(self, buf):
            ...
            display = buf if buf else 'waiting for input in VS Code...'
        ...
        def _request_completion(self, buf):
            ...
            if self.completing:
                return
            current = _completion_candidate_buffer(
                self.watcher.buffer,
                self._last_nonempty_buf,
                self.watcher.last_event_type,
            )
        ...
        def _request_sim_manual(self, event=None):
            ...
            buf = _completion_candidate_buffer(
                self.watcher.buffer,
                self._last_nonempty_buf,
                self.watcher.last_event_type,
            )
