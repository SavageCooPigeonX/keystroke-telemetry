...
def simulate_batch_rewrites(
    ...
    if write:
        ...
        result["deepseek_code_completion"]["daemon_once"] = _run_deepseek_code_completion_once(root, result)
        result["file_job_council"] = _file_job_council(root, result)
...
def _run_deepseek_code_completion_once(root: Path, result: dict[str, Any]) -> dict[str, Any]:
    queued = (result.get("deepseek_code_completion") or {}).get("count", 0)
    if not queued or os.environ.get("PIGEON_FILE_SIM_DEEPSEEK") == "0":
        return {"status": "skipped", "reason": "no queued jobs or disabled"}
    try:
        from src.local_env_loader_seq001_v001 import has_env_key, load_local_env
        load_local_env(root)
        if not has_env_key(root, "DEEPSEEK_API_KEY"):
            return {"status": "skipped", "reason": "missing_DEEPSEEK_API_KEY"}
    except Exception as exc:
        return {"status": "skipped", "reason": f"env_loader_error:{exc}"}
    try:
        completed = subprocess.run(
            ["py", "src/deepseek_daemon_seq001_v001.py", "--once"],
            cwd=root,
            text=True,
            capture_output=True,
            timeout=int(os.environ.get("PIGEON_FILE_SIM_DEEPSEEK_TIMEOUT", "120")),
            check=False,
            encoding="utf-8",
            errors="replace",
        )
    except subprocess.TimeoutExpired as exc:
        return {"status": "timeout", "stdout_tail": _tail(exc.stdout or ""), "stderr_tail": str(exc)}
    return {
        "status": "ran",
