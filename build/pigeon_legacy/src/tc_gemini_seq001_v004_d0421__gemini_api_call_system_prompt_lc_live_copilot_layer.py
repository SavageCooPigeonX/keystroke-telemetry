...
def _candidate_gemini_env_paths() -> list[Path]:
    """Known local key locations, ordered from repo-local to shared workspace."""
    explicit = os.environ.get('GEMINI_ENV_PATH') or os.environ.get('GOOGLE_API_ENV_PATH')
    paths = [
        Path(explicit) if explicit else None,
        ROOT / '.env',
        ROOT / 'logs' / 'gemini.env',
        ROOT.parent / '.env',
        ROOT.parent / 'kt-main-push' / '.env',
        ROOT.parent.parent / 'GitHub' / 'LinkRouter.ai' / '.env',
        Path.home() / 'OneDrive' / 'Documents' / 'GitHub' / 'LinkRouter.ai' / '.env',
        Path.home() / 'Documents' / 'GitHub' / 'LinkRouter.ai' / '.env',
    ]
    seen: set[str] = set()
    unique: list[Path] = []
    for path in paths:
...
def _load_api_key_details() -> tuple[str | None, str | None]:
    for env_path in _candidate_gemini_env_paths():
        if env_path.exists():
            for line in env_path.read_text('utf-8', errors='ignore').splitlines():
                ...
                if key in {'GEMINI_API_KEY', 'GOOGLE_API_KEY'}:
                    return value.strip().strip("'\""), str(env_path)
...
def _load_api_key() -> str | None:
    return _load_api_key_details()[0]


def _write_gemini_status(status: str, reason: str, buffer: str = "", ctx_names: list[str] | None = None) -> None:
    try:
        ...
        path.write_text(json.dumps({
            ...
            'api_key_source': _load_api_key_details()[1],
            'buffer_preview': buffer[:180],
            'context_files': ctx_names or [],
        }, ensure_ascii=False, indent=2), encoding='utf-8')
    except Exception:
        pass


def _local_no_key_completion(buffer: str, selected_files: list[dict]) -> str:
    names = [str(f.get('name') or '') for f in selected_files[:4] if f.get('name')]
    if names:
        return (
            f" - local pause fired but gemini is missing GEMINI_API_KEY; current intent wakes "
            f"{', '.join(names)} and should be routed through prompt brain / intent graph until the key is set"
        )
    return (
        " - local pause fired but gemini is missing GEMINI_API_KEY; no model completion can run, "
        "so fix the key or use local intent graph context only"
    )


def _fallback_context_files(buffer: str) -> list[dict]:
    """Context fallback when the legacy context agent returns no files."""
    lower = buffer.lower()
    rows: list[dict] = []
    hints = [
    ...
    for needles, name, score in hints:
        if any(needle in lower for needle in needles):
            rows.append({"name": name, "score": score, "sources": ["completion_fallback"]})
    if rows:
        return rows
    return [{"name": "thought_completer", "score": 0.35, "sources": ["completion_fallback_default"]}]
...
def call_gemini(buffer: str, thought_buffer: ThoughtBuffer | None = None) -> tuple[str, list[str]]:
    ...
    if not selected_files:
        selected_files = _fallback_context_files(buffer)
    ...
    if not api_key:
        _write_gemini_status('local_fallback', 'missing_GEMINI_API_KEY', buffer, ctx_names)
        return _local_no_key_completion(buffer, selected_files), ctx_names
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=GEMINI_TIMEOUT) as resp:
            ...
            try:
                ...
                pass
            _write_gemini_status('ok', 'completion_returned', buffer, ctx_names)
            return text, ctx_names
    except Exception as e:
        print(f'[completer] gemini error: {e}')
        _write_gemini_status('error', str(e)[:300], buffer, ctx_names)
        return '', ctx_names
