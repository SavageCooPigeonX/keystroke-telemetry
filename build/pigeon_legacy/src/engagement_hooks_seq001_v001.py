...
def _generate_hook_tuples(root, history=None, max_hooks=3,
    ...
    if priority_unsaid:
        ...
        selected.append((text, intensity, action))
...
def _target_intent_node(root):
    root = Path(root)
    graph = _json(root / "logs" / "intent_graph_latest.json") or {}
    matches = graph.get("intent_node_matches") if isinstance(graph.get("intent_node_matches"), list) else []
    if matches:
        first = matches[0] if isinstance(matches[0], dict) else {}
        return first.get("node_id") or first.get("node_key") or ""
    latest = _json(root / "logs" / "intent_nodes_latest.json") or {}
    touched = latest.get("nodes_touched") if isinstance(latest.get("nodes_touched"), list) else []
    if touched:
        first = touched[0] if isinstance(touched[0], dict) else {}
        return first.get("node_id") or first.get("node_key") or ""
    return ""


def _expected_operator_action(action):
    return {
        "dare": "pick the named file or approve the bounded mutation",
        "guilt": "surface whether the avoided file belongs in the next context pack",
        "taunt": "convert the pressure into one concrete file/action pair",
        "lure": "go deeper on the matched file cluster",
        "reveal": "name the unsaid intent or paste it into the next prompt",
        "diagnose": "confirm the failure mode or choose the file to inspect",
    }.get(action, "respond with the next bounded operator move")


def _hook_record(root, text, intensity, action):
    hook_id = "hook-" + hashlib.sha256(f"{action}|{text}".encode("utf-8")).hexdigest()[:12]
    return {
        "schema": "engagement_hook/v1",
        "hook_id": hook_id,
        "hook_type": action,
        "text": text,
        "intensity": intensity,
        "target_intent_node": _target_intent_node(root),
        "expected_operator_action": _expected_operator_action(action),
    }


def generate_hook_records(root, history=None, max_hooks=3,
                          min_intensity=1, max_intensity=6):
    root = Path(root)
    tuples = _generate_hook_tuples(
        root,
        history=history,
        max_hooks=max_hooks,
    ...
    records = [_hook_record(root, text, intensity, action) for text, intensity, action in tuples]
    latest = {
        "schema": "engagement_hooks_latest/v1",
    ...
    (logs / "engagement_hooks_latest.json").write_text(json.dumps(latest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    return records


def generate_hooks(root, history=None, max_hooks=3,
                   min_intensity=1, max_intensity=6):
    records = generate_hook_records(
        root,
        history=history,
        max_hooks=max_hooks,
        min_intensity=min_intensity,
...
def build_hooks_block(root, history=None):
    hook_records = generate_hook_records(root, history=history, max_hooks=5)
    hooks = [record["text"] for record in hook_records]
    if not hooks:
        return ""
    ...
    for record in hook_records:
        lines.append(
            f"- [{record['hook_id']} {record['hook_type']} intensity={record['intensity']}] {record['text']}"
        )
    lines.append("")
