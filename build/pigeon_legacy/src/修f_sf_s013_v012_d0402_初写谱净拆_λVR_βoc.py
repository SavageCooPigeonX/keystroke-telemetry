...
def _scan_over_hard_cap(root: Path, registry: dict) -> list[dict]:
    ...
    for entry in reg_list:
        ...
        abs_p = root / rel
        if abs_p.exists():
            candidates[rel.replace('\\', '/')] = abs_p

    # Registry drift is common during rename cascades. The compiler's hard cap
    # must be enforced against the actual tree too, or giant unregistered files
    # become invisible to self-fix.
    for abs_p in root.rglob('*.py'):
        try:
            rel = abs_p.relative_to(root).as_posix()
        except ValueError:
            continue
        candidates.setdefault(rel, abs_p)

    for rel, abs_p in sorted(candidates.items()):
        # Skip anything that should never be auto-compiled.
        if is_excluded(abs_p, root):
            continue
...
def run_self_fix(
    ...
    return {
        ...
        'total_bugs': len(all_problems),
        'over_cap': sum(1 for p in all_problems if p.get('type') == 'over_hard_cap'),
